...
We get reset vector $\NiceMatrixOptions{code-for-first-col=\scriptstyle}
\vec{r}_G = \ \ 
\begin{bNiceMatrix}[first-col=3]
	x & 1 \\
	y & 1 
\end{bNiceMatrix}
$
and addition vector $\NiceMatrixOptions{code-for-first-col=\scriptstyle}
\vec{a}_G = \ \ 
\begin{bNiceMatrix}[first-col=3]
	x & 2 \\
	y & -3 
\end{bNiceMatrix}$ \\
$G$ can therefore be modeled with the \qvasr  
$V_P = \begin{Bmatrix}
	\begin{pmatrix}
		\NiceMatrixOptions{code-for-first-col=\scriptstyle}
		\begin{bNiceMatrix}[first-col=3]
			x & 1 \\
			y & 1
		\end{bNiceMatrix},
...
\begin{bNiceMatrix}[first-row=1]
	...
	-1 & 1 & 1 \\
	0 & 1 & 0
\end{bNiceMatrix} $, in which the first row represents the relation $-x + y + z$ and the second row, stemming from the conjunct $y' = y + 1$, represents just $y$. The tuple $(S, V)$ is called a \textsl{\qvasr-abstraction}. For transition formulas with constant increments, such as $G$, an identity matrix $I$ is used as simulation matrix. \par
