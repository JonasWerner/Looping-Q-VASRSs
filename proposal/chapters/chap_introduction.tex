...
The proposed technique makes use of \textsl{rational vector addition systems} (\qvasr). A \qvasr of dimension $d$ is a finite set $V \subseteq \{0, 1\}^d \times \mathbb{Q}^d$ of transformers. Each transformer $(\vec{r}, \vec{a})$ consist of binary \textsl{reset} vector $\vec{r}$ and rational \textsl{addition} vector $\vec{a}$. These transformer model transitions between two vectors $\vec{u}$ and $\vec{v}$ the following way: $\vec{v} = \vec{u} * \vec{r} + \vec{a}$, with * being the Hadamard product.
\qvasr can be used to model transition formulas: \\
For example assume the following transition formula $F: x' = x + 1 \land y' = 2$ is given. 
$
\begin{pmatrix}
	\begin{pmatrix}
		1 \\
