...
\begin{tikzpicture}[%
	...
	\node [align=left, right of=predtransformer, xshift=3cm](output) {\textbf{Output}: \\
		loop summary};
	\node [block, align=left, below of=predtransformer](vasrs) {\texttt{\qvasrs-builder}};
	\node [block, align=left, right of=vasrs, xshift=3cm](reach) {\texttt{reach}};
	\node [block, align=left, below of=vasrs, yshift=-1cm](H) {Is $\Gamma$ satisfiable?};
	...
	\draw [->, bend right]  (vasrs) to node[left] {For every $p, q \in P:$\\$ \Gamma := p \land F \land q$ \\ \\ $S_\Gamma := \mathit{I_n}$ \\
	$V_\Gamma := \emptyset$} node[right, align=left] {} (H);
	\draw [->]  (H) to node[above] {yes} node[below] {$c :=$ cube in \\ $\mathit{DNF(\Gamma)}$} (abstr);
	\draw [->]  (abstr) to node[above] {} node[left] {$(S_c, V_c) :=$ abstraction(c)} (pushout);
	\draw [->]  (pushout) to node[above] {} node[above] {$S_\Gamma :=$ \texttt{pushout($S_\Gamma, S_c$)}} (image);
	\draw [->]  (image) to node[above] {} node[left, align=left] {$\Gamma :=$ refine$(\Gamma)$ \\ $V_\Gamma :=$ \texttt{image$(V_\Gamma)$} \\ $\cup$ \texttt{image$(V_c)$}} (H);
	\draw [->, bend right]  (H) to node[above] {} node[right, align=left] {no \\
	$(S_\Gamma, V_\Gamma)$ is best \qvasr-abstraction for $\Gamma$ \\ $\mathcal{A} := \mathcal{A} \cup \pqvasr$} (vasrs);
	\draw [->]  (vasrs) to node[above] {} node[above] {$\mathcal{A}$ is best \\ $\qvasrs$-abstraction \\ of $F$} (reach);
\end{tikzpicture}