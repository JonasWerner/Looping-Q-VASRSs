...
\begin{tikzpicture}[%
	...
	\node [block, align=center, below=of predtransformer, yshift=1.5cm](vasrs) {Are there $p, q \in P$ with no \\ \qvasr-abstraction \\ \pqvasr $\in \mathcal{V}$?};
	...
	\draw (reach) to node[above] {} node[left] {\texttt{reach($\mathcal{A}$)} = \\ loop summary} (output);
	...
	\draw (pushout) to node[above] {} node[above] {$\pq{S}$ simulates $S_c$} (image);
