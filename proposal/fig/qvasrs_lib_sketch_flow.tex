...
\begin{tikzpicture}[%
	...
	\node [align=left, yshift=-1.5cm](input) {\textbf{Input}: \\
		- transition formula $F$ of a loop};
	...
	\node [align=left, right of=predtransformer, xshift=3cm](output) {\textbf{Output}: \\
		- loop summary};
	\node [block, align=left, below of=predtransformer](vasrs) {\texttt{\qvasrs-engine}};
	\node [block, align=left, right of=vasrs, xshift=3cm](reach) {\texttt{reach}};
	\node [block, align=left, below of=vasrs](H) {Is $\Gamma$ satisfiable?};
	...
	% Draw edges
	\draw [->]  (input) to node[above] {} node[above] {} (predtransformer);
	\draw [->]  (predtransformer) to node[right] {} node[right, align=left] {$F$ \\ set of predicates $P$} (vasrs);
	\draw [->]  (reach) to node[above] {} node[above] {} (output);
	\draw [->]  (H) to node[above] {yes} node[below] {} (abstr);
	\draw [->]  (abstr) to node[above] {} node[right] {\qvasr-abstraction $\mathcal{A}$} (pushout);
	\draw [->]  (pushout) to node[above] {} node[above] {} (image);
	\draw [->]  (image) to node[above] {} node[right, align=left] {refine $\Gamma$} (H);
