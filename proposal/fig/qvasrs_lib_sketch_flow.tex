...
\begin{tikzpicture}[%
	...
	\draw [bend right]  (vasrs) to node[left, align=right] {\textbf{yes} \\ $\pqformula := p \land F \land q$ \\ \\ $\pqvasr := (\mathit{I_n},\emptyset)$} node[right, align=left] {} (H);
	\draw (H) to node[above] {\textbf{yes}} node[below] {$c :=$ cube in $\mathit{DNF(\pqformula)}$} (abstr);
	...
	%\draw (image) to node[above] {} node[left, align=left] {$\pqformula :=$ refine$(\pqformula)$ \\ $\pq{V} :=$ \texttt{image(\pq{V})} \\ $\cup$ \texttt{image$(V_c)$}} (H);
		...
		{\begin{varwidth}{3.5cm}
				\vspace*{-0.5cm}
				\begin{align*}
					\pqformula :=&\ \texttt{refine}(\pqformula) \\ \\
	...
	\draw [bend right]  (H) to node[above] {} node[right, align=left] {\textbf{no} \\
	$\mathcal{V} := \mathcal{V} \cup \pqvasr$} (vasrs);
	\draw (vasrs) to node[below] {$\mathcal{A} := \qvasrs(\mathcal{V}, P)$} node[above] {\textbf{no}} (reach);
\end{tikzpicture}