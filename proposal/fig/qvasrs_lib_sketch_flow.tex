...
\begin{tikzpicture}[%
	...
	\draw [->, bend right]  (vasrs) to node[left] {yes \\ $\pqformula := p \land F \land q$ \\ $\pqvasr := (\mathit{I_n},\emptyset)$} node[right, align=left] {} (H);
	...
	\draw [->]  (vasrs) to node[below] {$\mathcal{A} := \qvasrs(\mathcal{V}, P)$} node[above] {no} (reach);
\end{tikzpicture}