...
\begin{tikzpicture}[%
	...
	\node [align=center](input) {\textbf{Input}: \\ \color<2>{emblue} Program trace $\tau$ \color<2>{black}};
	\node[in node, fill=white](1)[below of=input, yshift=0.25cm]{\color<3-4>{emblue} Does $\tau$ contain a loop?};
	
	\node[my node, fill=white](2)[below of=1, xshift=1cm]{\color<5>{emblue}Loop summarizable?};
