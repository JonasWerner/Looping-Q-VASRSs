...
\tikzstyle{groupBig} = [rectangle, draw, rounded corners, minimum height=10em
]
...
\begin{tikzpicture}[%
	...
	\node [block, align=left, below=of input, fill=white](predtransformer) {\texttt{predicate transformer}};
	\node [block, align=center, right=of predtransformer, fill=white](vasrs) {Are there $p, q \in P$ with no \\ \qvasr-abstraction \\ \pqvasr $\in \mathcal{A}$?};
	\node [block, align=left, right=of vasrs, fill=white](reach) {\texttt{reach}};
	\node [block, align=left, below=of vasrs, yshift=1cm, fill=white](H) {\pqformula\ satisfiable?};
	\node [block, align=center, right=of H,fill=white](abstr) {\texttt{\qvasr-abstractor}};
	...
	\begin{scope}[on background layer]
	\node[groupBig, draw=black, fill=orange,fit=(predtransformer) (reach), label=above :{\color{orange}\qvasrs}](FIt1) {};
	\end{scope}

	\begin{scope}[on background layer]
	\node[group, draw=black, fill=green,fit=(H) (abstr), label=left: {\color{green}\qvasr-abstraction}](FIt1) {};
	\end{scope}
	
	\begin{scope}[on background layer]
		\node[group, draw=black, fill=teal,fit=(image) (pushout), label=left: {\color{teal}\qvasr\ partial order}](FIt1) {};
	\end{scope}
