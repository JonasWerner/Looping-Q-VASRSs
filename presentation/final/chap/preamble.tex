...
\usepackage{amsmath}
\usepackage{tikz}
\usetikzlibrary{positioning,shapes.geometric, arrows.meta ,automata,decorations, decorations.text, decorations.pathreplacing, calc, fit, backgrounds, quotes, tikzmark, spy}
\usepackage{pgf}
\usepackage{pgfplots}
\pgfplotsset{
	compat=1.9,
	log ticks with fixed point, % no scientific notation in plots
	table/col sep=comma, % only tabs are column separators
...
\lstdefinestyle{withAssert}{
	language=C++,basicstyle=\ttfamily,keywordstyle=\color{blue}, escapechar=\%, morekeywords={assert}
}