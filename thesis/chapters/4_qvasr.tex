...
A \qvasr can be used to represent an overapproximation of a given transition formula. A transition formula $F$ over variables $\vec{x} = x_1, \ldots, x_n$ and $\vec{x}' = x_1', \ldots, x_n'$ designate the program state before and after the transition, $n$ is called the dimension of the formula. Figure \ref{codeWithAss} depicts a program containing a \texttt{while} loop from lines 4 - 12 for which we want to compute a loop summary using \qvasr. \par
