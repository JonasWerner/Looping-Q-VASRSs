...
\begin{mydef}[\qvasr-Abstraction]
	...
	\begin{bmatrix}
		...
		s_{m ,1} & \ldots & s_{m, n} \\
	\end{bmatrix}$ 
	such that for all transitions $\vec{x} \rightarrow_\trf \vec{x}'$ we have $S\vec{x} \rightarrow_V S\vec{x}'$. Meaning, every transition $\vec{x} \rightarrow_\trf \vec{x}'$ can be represented in $V$ by a matrix multiplication of $S$ and $\vec{x}$ and $\vec{x}'$.
\end{mydef}
...
\begin{align*}
	Res_H: s_1(x + y) + s_2(y + 1) + s_3x = a
\end{align*}
\begin{align*}
	Inc_H: s_1(x + y) + s_2(y + 1) + s_3x = s_1x + s_2y + s_3z + a
\end{align*}
resulting in:
\vspace*{-0.5em}
\begin{center}
	...
	\end{minipage}
\end{center}
These sets now from a vector space, and, because we want to construct a linear combinations of variables that are reset and incremented along $H$, we need to compute a basis for each space:
\vspace*{-1em}
\begin{center}
	\begin{minipage}{0.5\linewidth}
		\begin{equation*}
