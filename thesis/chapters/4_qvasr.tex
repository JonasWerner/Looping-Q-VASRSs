...
\subsection{\qvasr}
To summarize a loop, we make use of a special kind of vector addition system. Vector additions systems are a class of infinite-state transition systems.
...
\begin{mydef}[Rational Vector Addition System]
	Given two vectors consisting of rational numbers $\vec{u}$, and $\vec{v}$ of length $d$, a rational vector addition system $V$ of dimension $d$ is a finite set $V \subseteq \mathbb{Q}^d$ of transformers. It defines a transition system \transSys{V}, with state space $S_V = \mathbb{Q}$ and transition relation $\vec{u} \rightarrow_V \vec{v}$ if and only if $\vec{v} = \vec{u} + \vec{a}$ for some $\vec{a} \in V$. 
\end{mydef}
